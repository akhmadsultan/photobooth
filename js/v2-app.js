...
const App = (() => {
  ...
  async function _runCapture(){
    ...
    //
    // KEY INSIGHT: The burn canvas MUST have the SAME aspect ratio as the
    // stickerOverlay element on screen. If they differ, sx ≠ sy in
    // burnToCanvasAsync and stickers squash. So:
    //   1. Read the overlay's actual pixel dimensions from the DOM.
    //   2. Create a burn canvas at 2× those dimensions (same ratio, higher res).
    //   3. Draw the raw camera image with object-fit:cover into it (same view
    //      the user sees in the preview).
    //   4. Burn stickers — now sx === sy → no squash, ever.
    let finalDataURL = dataURL;
    try {
      // Step A: Get the actual overlay dimensions on screen
      const overlayEl2 = document.getElementById('stickerOverlay');
      const fr2 = overlayEl2?.getBoundingClientRect();
      const ovW = fr2?.width  || 640;
      const ovH = fr2?.height || 480;

      // Step B: Load raw camera image
      const srcImg = await new Promise(res => {
        const i = new Image(); i.onload = () => res(i); i.src = dataURL;
      });

      // Step C: Create burn canvas matching overlay aspect ratio (at 2× res)
      const OUT_W = Math.max(Math.round(ovW * 2), 640);
      const OUT_H = Math.round(OUT_W * ovH / ovW);
      const burnCvs = document.createElement('canvas');
      burnCvs.width  = OUT_W;
      burnCvs.height = OUT_H;
      const burnCtx  = burnCvs.getContext('2d');

      // Step D: Draw camera image with object-fit:cover semantics
      // (matches exactly what the user sees in the camera preview)
      const camW = srcImg.naturalWidth;
      const camH = srcImg.naturalHeight;
      const scale = Math.max(OUT_W / camW, OUT_H / camH);
      const drawW = camW * scale;
      const drawH = camH * scale;
      const drawX = (OUT_W - drawW) / 2;
      const drawY = (OUT_H - drawH) / 2;
      burnCtx.drawImage(srcImg, drawX, drawY, drawW, drawH);

      // Step E: Burn stickers — sx = OUT_W/ovW, sy = OUT_H/ovH are EQUAL
      // because OUT_W/OUT_H === ovW/ovH by construction → zero squash
  ...
  async function _onComplete(){
    ...
    if (stripCanvas) stripCanvas.style.display = 'block';
    
    // Hide manual output blocks initially
    ...
    _setStatus('active', 'Generating Boomerang...');
    U.toast('Membuat Boomerang GIF... 🎬', 'info', 3000);
    
    // Trigger GIF generation in the background automatically
    const frames = Strip.getFrames();
    let gifBlob = null;
    try {
      const gifURL = await GifMaker.generate(frames);
      
      // Update GIF preview if applicable
      const gifOut = document.getElementById('gifOut');
      if (gifOut) {
        gifOut.src = gifURL;
        gifOut.style.display = 'block';
      }
      const gifLoadState = document.getElementById('gifLoadState');
      if (gifLoadState) gifLoadState.style.display = 'none';
      
      // Convert GIF blob URL to actual Blob
      const res = await fetch(gifURL);
      gifBlob = await res.blob();
    } catch (e) {
      console.warn('[AutoSave] GIF generation failed:', e);
      U.toast('Peringatan: Gagal membuat Boomerang GIF.', 'warn', 3000);
    }
    
    // Upload photo strip, individual frames, and GIF
    _setStatus('active', 'Uploading to Google Drive...');
    U.toast('Mengunggah hasil sesi ke Google Drive... 🚀', 'info', 4000);
    
    try {
      const stripURL = await Strip.getDataURL();
      if (!stripURL) throw new Error('Gagal merender strip foto');
      ...
      if (res.success) {
        window.SFX?.success?.();
        U.toast(`Berhasil disimpan!`, 'success', 3000);
        
        if (res.gdrive_link && res.gdrive_link !== 'null') {
          _showQrModal(res.gdrive_link, res.gdrive_configured);
        } else {
          // GDrive link is null (not configured or upload failed)
          // Fallback: show manual options since GDrive is unconfigured
          _showManualFallback();
          if (res.gdrive_configured) {
            const errs = (res.errors || []).join('; ');
            U.toast('⚠️ GDrive gagal: ' + (errs || 'Silakan cek console server Anda.'), 'error', 7000);
          } else {
            U.toast('Google Drive belum dikonfigurasi. Silakan download manual.', 'info', 5000);
          }
        }
      } else {
      ...
      window.SFX?.error?.();
      U.toast('Gagal mengunggah otomatis: ' + e.message, 'error', 7000);
  ...
  function _showManualFallback() {
    ...
    const dlGifBtn = document.getElementById('btnDlGif');
    if (dlGifBtn) dlGifBtn.style.display = 'block';
    _setStatus('active', 'Simpan/Download manual');
  }
  ...
  function _bindButtons(){
    ...
    document.getElementById('btnSaveGal')?.addEventListener('click', async () => {
      ...
      try {
        ...
        let gifBlob = null;
        const gifURL = GifMaker.getURL();
        if (gifURL) {
          try {
            const gifRes = await fetch(gifURL);
            gifBlob = await gifRes.blob();
          } catch (err) {
            console.warn('[SaveGallery] Failed to fetch GIF blob:', err);
          }
        }
        ...
        if (res.success) {
          window.SFX?.success?.();
          U.toast(`Saved! Session: ${res.id}`, 'success', 3000);
          if (res.gdrive_link && res.gdrive_link !== 'null') {
            _showQrModal(res.gdrive_link, res.gdrive_configured);
          } else if (res.gdrive_configured) {
            // Credentials exist but GDrive call failed (API not enabled, etc.)
            const errs = (res.errors || []).join('; ');
            U.toast('⚠️ GDrive gagal: ' + (errs || 'Cek apakah Google Drive API sudah diaktifkan di Google Cloud Console.'), 'error', 10000);
          }
        } else {
          throw new Error(res.message || 'Unknown server error');
        }
    ...
    document.getElementById('btnQrNewSession')?.addEventListener('click', () => {
      window.SFX?.click?.();
      if(confirm('Mulai sesi baru? Foto saat ini akan hilang.')) location.reload();
    });
  }
  ...
  /* ── Google Drive QR Code Modal display ────────────────── */
  function _showQrModal(link, configured) {
    // Guard: never show modal if link is null/undefined/empty
    if (!link || link === 'null') {
      console.warn('[QR Modal] No valid gdrive_link, modal skipped.');
      return;
    }
    ...
    const qrDemoBadge = document.getElementById('qrDemoBadge');
    ...
    if (qrDirectLink) {
      qrDirectLink.href = link;
      qrDirectLink.textContent = configured ? 'Buka Link Google Drive secara langsung' : 'Buka Link Simulasi secara langsung';
    }

    if (qrDemoBadge) {
      qrDemoBadge.style.display = configured ? 'none' : 'block';
    }
