...
const Stickers = (() => {
  ...
  async function burnToCanvasAsync(canvas) {
    ...
    // sx/sy: how many canvas pixels per overlay pixel
    const sx = canvas.width  / fr.width;
    const sy = canvas.height / fr.height;
    // Use geometric mean for SIZE mapping (handles any residual ratio difference)
    const ss = Math.sqrt(sx * sy);

    await Promise.all(stickers.map(data => new Promise(resolve => {
      const img  = new Image();

      const draw = () => {
        // Position uses separate sx/sy (maps overlay coords → canvas coords)
        const cx = data.x * sx;
        const cy = data.y * sy;
        // The allocated square on the canvas (same scale as overlay square)
        const boxW = data.size * ss;
        const boxH = data.size * ss;

        // ── object-fit: contain semantics ──────────────────────────
        // CSS in DOM uses object-fit:contain for img stickers, so we must
        // replicate that here or portrait/landscape stickers will squash.
        const natW = img.naturalWidth  || img.width  || data.size;
        const natH = img.naturalHeight || img.height || data.size;
        const imgRatio = natW / natH;
        let drawW, drawH;
        if (imgRatio > 1) {
          // Landscape image: fit to box width
          drawW = boxW;
          drawH = boxW / imgRatio;
        } else {
          // Portrait or square image: fit to box height
          drawH = boxH;
          drawW = boxH * imgRatio;
        }
        // Center within the allocated box (same as contain behaviour)
        const offX = (boxW - drawW) / 2;
        const offY = (boxH - drawH) / 2;

        ctx.save();
        // Translate to center of the allocated box
        ctx.translate(cx + boxW / 2, cy + boxH / 2);
        ctx.rotate(data.rotation * Math.PI / 180);
        // Draw centered at natural aspect ratio (object-fit: contain)
