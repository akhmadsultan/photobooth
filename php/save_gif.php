...
$gdriveGifId = null;
$gdriveError = null;
if (gdrive_is_configured() && !empty($meta['gdrive_folder_id'])) {
    $folderId = $meta['gdrive_folder_id'];
    $upResult = gdrive_upload_file($destPath, 'boomerang.gif', 'image/gif', $folderId);
    if (isset($upResult['error'])) {
        $gdriveError = $upResult['error'];
    } else {
        $gdriveGifId = $upResult['id'];
        $meta['gdrive_gif_id'] = $gdriveGifId;
    }
}
